...
def draw_graph(path, prefix, config, edges_by_name, names, types, value_att, FONT_SCALE):
    ...
    node_ratio = 1000 / max(list(freq_dict_by_name.values()))
    ...
    dpi = 200
    width = 0.5
    nx.draw_networkx_edges(G, pos, edgelist=edges_by_name, arrows=False, width=width, alpha=0.2)
    ...
    font_ratio = max_font / (math.log(max(value_att), FONT_SCALE) + 1)
